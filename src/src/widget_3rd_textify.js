...
widgets.textify = { 
  ...
  resources: {
    style_head     : "html {background:#ff0;} body { text-align:center; margin;auto; }  canvas { border:5px solid black}  ",
  ...
  register: function (slideDoc) { 
	...
	var obj =  {   
		markup_menu: "<button style='background:#ff0' id='textify_do'>Textify</button>",
  ...
  init: function () { 
	...
	jQuery("#widgetscanvas",this.slideDoc).css("background-color","#ecc");
        jQuery("#widgetscanvas",this.slideDoc).html("<div class='textify'>Select area from the taken page screenshot.</div>");

	this.edited = false; 
	this.editor.on = false; 

	widgets.textify.referenceContentWindow = jetpack.tabs.focused.contentWindow;

	this.canvasTab    = jetpack.tabs.open("about:blank");
	this.canvasTab.focus();
	this.canvasTab.onReady(function(doc) { 

		var namedRefThis = widgets.textify; 
		...
		xWid.dump("textify:"+ namedRefThis.referenceContentWindow);

		// Take the screenshot! 
		// widgets.textify.createPreviewRaw(namedRefThis.referenceContentWindow, namedRefThis.canvas);
  ...
  editorSnap: function () {
	...
	jQuery("body",this.editorDoc).append("<h2>Screenshot selected!</h2><p>Click Send to send this image to your history <button id='widget_textify_addbutton'>Send</button> or simply close this tab to give up. ");
        jQuery("#widgetscanvas",this.slideDoc).html("Click the Send button to add the image. <button id='widget_textify_send'>Send</button>");
		...
		xWid.digester.add(widgets.textify, widgets.textify.canvas.toDataURL("image/png",""));
        })

	jQuery("#widget_textify_addbutton",this.editorDoc).click( function () { 
                jQuery("#widgetscanvas",refThis.slideDoc).html("");
                jQuery("#widgetscanvas",refThis.slideDoc).css("display","none");
		xWid.digester.add(widgets.textify, widgets.textify.canvas.toDataURL("image/png",""));
...
//widgets.list.push(widgets.textify); 
widgets.list[widgets.textify.name] = widgets.textify;

xWid.cssStack_slidebar.push(".statement img { width:64px; } #widgetscanvas .textify { padding:.5em }  ");
